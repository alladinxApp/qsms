<?
	...
	if(isset($_POST['search']) && !empty($_POST['search']) && $_POST['search'] == 1){
		...
		$where = "WHERE trans_status IN('4','5','8') AND wo_refno <> '0'";
		if(!empty($datefrom) && !empty($dateto)){
			$datefrom = dateFormat($datefrom,"Y-m-d");
			$dateto = dateFormat($dateto,"Y-m-d");
			$where .= " AND (wo_trans_date between '$datefrom 00:00' AND '$dateto 23:59')";
...
<body>
	...
	<table width="2200">
		...
		<tr>
			...
			<td style="<?=$bg . $color;?>"><a href="workorder_approval.php?estimaterefno=<?=$rowservices['estimate_refno'];?>"><?=$rowservices['estimate_refno'];?></a></td>
