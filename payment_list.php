...
<script type="text/javascript">
	...
	function SearchPayment(){
		var data = document.getElementById("txtsearchpayment").value;
		if(data == ""){
			alert("Please enter payment to search!");
			return false;
		}
		var strURL = "divSearchPayment.php?data="+data;
		var req = getXMLHTTP();
		if (req){
			req.onreadystatechange = function(){
				if (req.readyState == 4){
						document.getElementById('divSearchPayment').innerHTML = req.responseText;
				}
			}
...
<body>
<div class="button"><a href="payment_add.php"><img src="images/add.jpg" /></a></div>

<div class="title">PAYMENT TYPE LIST</div> 

Search by Payment: <input type="text" name="txtsearchpayment" id="txtsearchpayment" name="search" placeholder="Enter Payment" /><input type="button" value="Search" onclick="SearchPayment();" />

<?Php

$page_name="payment_list.php"; //  If you use this code with a different page ( or file ) name then change this 

@$column_name=$_GET['payment_list']; // Read the column name from query string. 
...
$query2=" SELECT * FROM v_payment  ";
$count=$dbo->prepare($query2);
$count->execute();
$nume=$count->rowCount();

echo "<span id='divSearchPayment'><TABLE><tr>";
...
$query=" SELECT * FROM v_payment  ";

if(isset($column_name) and strlen($column_name)>0){
$query = $query . " order by $payment_id";
}
