<?
	...
	if(isset($_POST['search']) && !empty($_POST['search']) && $_POST['search'] == 1){
		...
		$where = "WHERE (tbl_rr_mst.status = '10' OR tbl_rr_mst.status = '1') ";
		...
		if(!empty($_POST['txtdatefrom']) || !empty($_POST['txtdateto']) || $cnt == 0){
			...
			$where .= " AND (v_po_mst.rr_post_date between '$datefrom' AND '$dateto') ";
		}		
...
<body>
	...
	<table width="1250">
		...
		<?
			...
			foreach($respomst as $rowpomst){
				if(!empty($rowpomst['payment_date'])) {
					$statdesc = "BILLED";
					$color = "color: #00ff00;";
				}else{
					$statdesc = "POSTED";
	...
	<table>
		...
		<tr>
			<td >CV Ref No</td>
			<td align="center">:</td>
			<td colspan="3"><input type="text" id="txtcvrefno" name="txtcvrefno" style="width: 230"></td>
		</tr>
		<tr>
			<td >&nbsp;</td>
			<td align="center">&nbsp;</td>
