<?
	...
	if(isset($_POST['search']) && !empty($_POST['search']) && $_POST['search'] == 1){
		...
		$qryservices = "SELECT * FROM v_online_estimate_master WHERE 1 " . $where . ' ORDER BY status,transaction_date DESC';
...
<body>
	...
	<p id="title">Online Estimate LIST</p>
	<div class="divEstimateList">
	<table width="2200">
		<tr>
			<th width="20">#</th>
			<th width="160">Online Estimate#</th>
		...
		<tr>
			...
			<td style="<?=$bg . $color;?>"><a style="<?=$color;?>" href="online_estimate_edit.php?estimaterefno=<?=$rowservices['oe_id'];?>"><?=$rowservices['oe_id'];?></a></td>
			<td align="center" style="<?=$bg . $color;?>"><?=$rowservices['transaction_date'];?></td>
			<td style="<?=$bg . $color;?>"><?=$rowservices['customer'];?></td>
			<td style="<?=$bg . $color;?>"><?=$rowservices['address'];?></td>
			<td align="center" style="<?=$bg . $color;?>"><?=$rowservices['plateno'];?></td>
			<td align="center" style="<?=$bg . $color;?>"><?=$rowservices['make'];?></td>
			<td align="center" style="<?=$bg . $color;?>"><?=$rowservices['year'];?></td>
			<td align="center" style="<?=$bg . $color;?>"><?=$rowservices['model'];?></td>
			<td align="center" style="<?=$bg . $color;?>"><?=$rowservices['color'];?></td>
			<td style="<?=$bg . $color;?>"><?=$rowservices['variant'];?></td>
			<td style="<?=$bg . $color;?>"><?=$rowservices['engineno'];?></td>
			<td style="<?=$bg . $color;?>"><?=$rowservices['chassisno'];?></td>
	...
	<? }else{ ?>
	<p id="title">Search Online Estimate LIST</p>
	<form method="Post">
