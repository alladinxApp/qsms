<?
	...
	class PrintRR extends FPDF{
		...
		public function Header(){
			...
			$this->Cell(190,4,'RECEIVING ORDER',0,0,'R');
			$this->Ln(10);
			...
			for($i=0;$i<count($this->mst['rr_ref_date']);$i++){
				switch($i){
					case 0:
							$this->SetFont('Courier','B',8);
							$this->Cell(30,4,'RR Reference/Date',0,0,'L');
							$this->Cell(5,4,':',0,0,'C');
							$this->SetFont('Courier','',8);
							$this->Cell(85,4,$this->mst['rr_ref_date'][$i],0,0,'L');
							$this->Ln();
						break;
					default:
		...
		public function ImprovedTable(){
			...
			$this->Cell(75,4,'DESCRIPTION',"LTRB",0,'C');
			$this->Cell(25,4,'PRICE',"LTRB",0,'C');
			$this->Cell(40,4,'QUANTITY',"LTRB",0,'C');
			$this->Cell(25,4,'TOTAL',"LTRB",0,'C');
			$this->Ln();
			...
			for($i=0;$i<count($this->dtl);$i++){
				...
				$this->Cell(75,4,$this->dtl[$i]['item_desc'],"LR",0,'L');
				$this->Cell(25,4,number_format($this->dtl[$i]['price'],2),"LR",0,'R');
				$this->Cell(40,4,$this->dtl[$i]['rr_qty'],"LR",0,'C');
				$this->Cell(25,4,number_format($this->dtl[$i]['total'],2),"LR",0,'R');
				$this->Ln();
			...
			$this->SetFont('Courier','B',8);
			$this->Cell(125,8,'TOTAL >>>>>>>>>>',"LTRB",0,'R');
			$this->Cell(40,8,$this->totalrrqty,"LTRB",0,'C');
			$this->Cell(25,8,number_format($this->total,2),"LTRB",0,'R');
			$this->Ln();

			$this->Cell(125,0,'',"LRB",0,'C');
			$this->Cell(40,0,'',"LRB",0,'C');
			$this->Cell(25,0,'',"LRB",0,'C');
			$this->Ln(4);
