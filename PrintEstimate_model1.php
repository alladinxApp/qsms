<?
	...
	class PrintEstimate extends FPDF{
		...
		private function TableHeader(){
			...
			$this->Cell(20,6,'QTY','LRB',0,'C');
			$this->Cell(20,6,'UNIT','LRB',0,'C');
			$this->Cell(90,6,'DESCRIPTION','LRB',0,'C');
		...
		public function Header(){
			...
			$this->SetTextColor(0,0,0);
			$this->Cell(50,6,'ESTIMATE REF',0,0,'L');
			$this->Ln();
			...
			$this->Cell(50,6,'No. ' . $this->estimaterefno,0,0,'L');
			$this->Ln();
		...
		private function AccessoryData($w){
			...
			$this->SetFont('Courier','',10);
			foreach($data as $row){
				$this->Cell($w[0],6,$row['qty'],'LR',0,'C');
				$this->Cell($w[1],6,'PC','LR',0,'C');
				$this->Cell($w[2],6,$row['accessory_name'] . $cntdata,'LR',0,'L');
				$this->Cell($w[3],6,number_format($row['amount'],2),'LR',0,'R');
				
				$total_accessory += $row['amount'];
				$this->Cell($w[4],6,number_format($total_accessory,2),'LR',0,'R');
				$this->Ln();
			}
		...
		private function JobData($w){
			...
			foreach($data as $row){
				$this->Cell($w[0],6,$row['qty'],'LR',0,'C');
				$this->Cell($w[1],6,'LABOR','LR',0,'C');
				$this->Cell($w[2],6,$row['job_name'],'LR',0,'L');
				$this->Cell($w[3],6,number_format($row['amount'],2),'LR',0,'R');
				
				$total_job += $row['amount'];
				$this->Cell($w[4],6,number_format($total_job,2),'LR',0,'R');
				$this->Ln();
			}
		...
		private function MakeData($w){
			...
			foreach($data as $row){
				$this->Cell($w[0],6,$row['qty'],'LR',0,'C');
				$this->Cell($w[1],6,'LABOR','LR',0,'C');
				$this->Cell($w[2],6,$row['make_name'],'LR',0,'L');
				$this->Cell($w[3],6,number_format($row['amount'],2),'LR',0,'R');
				
				$total_make += $row['amount'];
				$this->Cell($w[4],6,number_format($total_make,2),'LR',0,'R');
				$this->Ln();
			}
		...
		private function MaterialData($w){
			...
			foreach($data as $row){
				$this->Cell($w[0],6,$row['qty'],'LR',0,'C');
				$this->Cell($w[1],6,'PC','LR',0,'C');
				$this->Cell($w[2],6,$row['material_name'],'LR',0,'L');
				$this->Cell($w[3],6,number_format($row['amount'],2),'LR',0,'R');
				
				$total_material += $row['amount'];
				$this->Cell($w[4],6,number_format($total_material,2),'LR',0,'R');
				$this->Ln();
			}
		...
		private function PartsData($w){
			...
			foreach($data as $row){
				$this->Cell($w[0],6,$row['qty'],'LR',0,'C');
				$this->Cell($w[1],6,'PC','LR',0,'C');
				$this->Cell($w[2],6,$row['parts_name'],'LR',0,'L');
				$this->Cell($w[3],6,number_format($row['amount'],2),'LR',0,'R');
				
				$total_parts += $row['amount'];
				$this->Cell($w[4],6,number_format($total_parts,2),'LR',0,'R');
				$this->Ln();
			}
