<?php
	...
	foreach($resparts as $row){
		...
		$new_parts_price = $row['new_parts_price'];
		$old_price_date = $row['new_price_date'];
	}
	
	if (isset($_POST['update'])){
		$parts = mysql_real_escape_string($_POST['parts']);
		...
		if(empty($_POST['new_part_srp'])){
			$parts_update = "UPDATE tbl_parts SET 
				parts='$parts',  
				parts_discount='$parts_discount', 
				part_srp='$part_srp', 
				part_onhand='$part_onhand',  
				parts_lowstock='$parts_lowstock' WHERE parts_id = '$parts_id'  ";
		}else{
			$new_price = $_POST['new_part_srp'];
...
<body>
	...
	<table>
		...
		<tr>
			...
			<td class="input" colspan="3"><input type="text" name="parts" id="parts" value="<?=$parts;?>" readonly style="width:272px"></td>
		</tr> 
		...
		<tr>
			<td class="label"><label name="lbl_part_srp">New Standard Retail Price:</label></td>
			<td class="input" colspan="3"><input type="text" name="new_part_srp" id="new_part_srp" value="<?=$new_parts_price;?>" onBlur="return CurrencyFormatted('part_srp'); return IsNumeric(this.value);" style="width:272px"></td> 
		</tr>
