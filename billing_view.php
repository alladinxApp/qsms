<?
	...
	foreach($resbillingdtl2 as $rowbillingdtl){
		...
		$po_refnos[] = $rowbillingdtl['po_refno'];
	}
	...
	if(isset($_POST['postbill']) && !empty($_POST['postbill']) && $_POST['postbill'] == 1){
		...
		for($i = 0;$i < count($wo_refnos);$i++){
			$sql .= "UPDATE tbl_service_master SET trans_status = '$wo_refnos[$i]'; ";
		}
		for($i = 0;$i < count($po_refnos);$i++){
			$sql .= "UPDATE tbl_po_master SET trans_status = '$po_refnos[$i]'; ";
		}
		
		$res = mysql_query($sql);
		if(!$res){
			echo '<script>alert("There has been an error on updating your work order! Please double check all the data and save.");</script>';
		}else{
			echo '<script>alert("Work Order successfully billed.");</script>';
		}
		echo '<script>window.location="billing_view.php";</script>';
	}
?>
<html>
<head>
