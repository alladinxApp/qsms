...
<script type="text/javascript">
	...
	function SearchItem(){
		var data = document.getElementById("txtsearchitem").value;
		if(data == ""){
			alert("Please enter item description to search!");
			return false;
		}
		var strURL = "divSearchItem.php?data="+data;
		var req = getXMLHTTP();
		if (req){
			req.onreadystatechange = function(){
				if (req.readyState == 4){
						document.getElementById('divSearchItem').innerHTML = req.responseText;
				}
			}
...
<div class="button"><a href="item_add.php"><img src="images/add.jpg" /></a></div>

<div class="title">ITEM LIST</div> 

Search by item description: <input type="text" name="txt" id="txtsearchitem" name="search" placeholder="Enter Item Description" /><input type="button" value="Search" onClick="SearchItem();" />

<?Php

$page_name="items_list.php"; //  If you use this code with a different page ( or file ) name then change this 

@$column_name=$_GET['items_list']; // Read the column name from query string. 
...
$query2=" SELECT * FROM v_items  ";
$count=$dbo->prepare($query2);
$count->execute();
$nume=$count->rowCount();

echo "<span id='divSearchItem'><TABLE><tr>";
...
$query = $query . " order by $item_code";
}
